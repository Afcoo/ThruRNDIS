...
- VM assets are selected during first-run onboarding and can later be changed
  in Settings. A valid generated asset folder includes the kernel, RTPVM
  initramfs, and parseable `wireguard/wg-server.conf` and
  `wireguard/wg-client.conf` files.
