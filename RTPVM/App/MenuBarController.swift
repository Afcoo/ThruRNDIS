...
final class MenuBarController: NSObject, NSMenuDelegate {
    ...
    private func updateStatusButton() {
        ...
        let image = NSImage(
            ...
            accessibilityDescription: "RTPVM status"
        )
        image?.isTemplate = true
        button.image = image
        button.toolTip = "RTPVM — VM \(store.vmDisplayState.rawValue), \(usbStatusTitle)"
    }
    ...
    private func rebuildMenu() {
        ...
        menu.addItem(.separator())
        menu.addItem(actionItem(title: "Quit RTPVM", action: #selector(quit), keyEquivalent: "q"))
    }
