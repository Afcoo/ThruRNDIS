...
struct USBAttachmentPrompt: Identifiable {
    ...
    var message: String {
        switch kind {
        case .attach:
            return "USB device \(accessory.usbIDText) is available to RTPVM. Start the VM if needed and attach this device?"
        case .replace(_, let previousUSBIDText, let isCurrentlyAttached):
            ...
            return "USB device \(previousUSBIDText) \(previousState). RTPVM will detach it if needed, restart the VM, and attach \(accessory.usbIDText). Continue?"
        case .assetsRequired:
