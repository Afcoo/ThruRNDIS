...
1. 압축 해제한 `script` 폴더에서 `make_vm_assets`를 실행해 vm asset을 생성합니다.

```sh
./make_vm_assets
```

2. `ThruRNDIS.app`을 실행합니다.
...
8. `WireGuard` 화면에서 host `.conf`를 복사하거나 저장합니다.

9. WireGuard 도구를 설치합니다.
...
- `ThruRNDIS`: SwiftUI app, VM/USB/WireGuard orchestration.
